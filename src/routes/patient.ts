...
import { PrismaClient, Patient, MotifConsultation, HygieneBuccoDentaire, TypeMastication, Profession,ActionType } from '@prisma/client';

const router = express.Router();
const prisma = new PrismaClient();

// Define ActionType enum locally since it's not being properly exported
...
// Router type helper function to avoid type errors with route handlers
...
type RequestHandler<P extends ParamsDictionary = ParamsDictionary, ResBody = any, ReqBody = any> = 
  (req: Request<P, ResBody, ReqBody>, res: Response, next: NextFunction) => void | Promise<void>;
